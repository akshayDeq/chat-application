  ...
  io.on("connection", (socket) => {
    socket.on("room", async ({ username, roomname }) => {
      try {
        ...
        const roomFound = await Room.findOne({ roomname });
        if (roomFound) {
          await Room.updateOne(
            { roomname },
            { $push: { connectedusers: { _id: socket.id, username } } }
          );
        } else {
          let newRoom = await Room.create({
            roomname,
            connectedusers: [{ _id: socket.id, username, admin: true }],
          });
          await newRoom.save();
        }
      } catch (err) {
    ...
    socket.on("chat message", async ({ message, username, roomname }) => {
      try {
        ...
        const user = room.connectedusers.filter((val) => {
    ...
    socket.on("leave room", async ({ roomname }) => {
      try {
        ...
        let room = await Room.findOne({ roomname });
        const newArray = room.connectedusers.filter((val) => {
